...
def cleanup_ddp_simple():
    """Clean up DDP."""
    if dist.is_initialized():
        dist.destroy_process_group()


def reduce_tensor_simple(tensor, world_size):
...
if __name__ == '__main__':
    ...
    # Initialize DDP first if available
    rank, world_size, device, is_main = setup_ddp_simple()
    ...
    if world_size > 1:
        ...
        if is_main:
            print(f"[DDP] Using DistributedSampler with {world_size} processes", flush=True)
            print(f"[DDP] Effective batch size: {batch_size * world_size}", flush=True)
    else:
    ...
    if is_main and not os.path.exists(checkpoint_folder):
    ...
    # Helper function to get model state dict (handles DDP and DataParallel)
    ...
    for epoch in range(start_epoch, num_epochs):
        # Set epoch for distributed samplers (ensures different shuffle each epoch)
        ...
        train_pbar = tqdm(enumerate(loader_train, 0),
                          unit="batch",
                          total=len(loader_train)) if is_main else enumerate(loader_train, 0)
        ...
        # Average training loss and reduce across processes
        if num_batches > 0:
            train_loss = train_loss / num_batches
        else:
            train_loss = 0.0
        
        # Reduce train loss across all processes
        if world_size > 1:
            train_loss_tensor = torch.tensor([train_loss], device=device)
            train_loss_tensor = reduce_tensor_simple(train_loss_tensor, world_size)
        ...
        val_pbar = tqdm(enumerate(loader_val, 0),
                        unit="batch",
                        total=len(loader_val)) if is_main else enumerate(loader_val, 0)
        ...
        # Average validation loss and reduce across processes
        if val_batches > 0:
            val_loss = val_loss / val_batches
        else:
            val_loss = 0.0
        
        # Reduce val loss across all processes
        if world_size > 1:
            val_loss_tensor = torch.tensor([val_loss], device=device)
            val_loss_tensor = reduce_tensor_simple(val_loss_tensor, world_size)
        ...
        # Early stopping (broadcast from main process)
        early_stop_signal = torch.tensor([0], device=device)
        if is_main:
            if(early_stopper.early_stop(val_loss - train_loss)):
                print("Early stopping!")
                early_stop_signal = torch.tensor([1], device=device)
    ...
    # Final saving (only main process)
    if is_main:
